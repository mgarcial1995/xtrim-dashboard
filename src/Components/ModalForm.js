...
function ModalForm({save, cancel, title, children}) {
    return (
        <div className="fixed w-full h-full top-0 left-0 z-50" >
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 max-w-xl flex rounded-lg flex-col p-6 h-auto bg-white shadow-md shadow-slate-900 z-10">
                <p className="text-left text-[#222D44] text-2xl font-bold">{title}</p>
                ...
                <div className={`w-full h-auto flex justify-end gap-x-4 mt-10`}>
                    ...
                    <Button click={save} text="Guardar" type="color" />
                </div>
            </div>
        </div>
        );
